...
def my_parse_args():
    ...
    parser.add_argument("--time_series_id_column_names", nargs="*", type=str, required=True)
...
def split_full_for_forecasting(
    df, time_column_name, split, grain_column_names=None
):
...
def run(mini_batch):
    ...
    for file in mini_batch:
        ...
        grain_array = file.split(f".{file_ext}")[0].split('/')[-no_of_grain_columns - 1: -1]
        ...
        if test_split > 0:
            ...
            test_df.to_parquet(test_path / "part-0.parquet", compression=None, index=False)
        else:
            train_df = df
        if valid_split > 0:
            ...
            valid_df.to_parquet(valid_path / "part-0.parquet", compression=None, index=False)

        train_path = output_dir / Path("train") / Path(grain_dir)
        train_path.mkdir(parents=True, exist_ok=True)
        train_df.to_parquet(train_path / "part-0.parquet", compression=None, index=False)          
