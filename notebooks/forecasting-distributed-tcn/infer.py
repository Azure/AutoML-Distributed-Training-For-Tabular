...
X_rf = fitted_model.rolling_forecast(X_test_df, y_test, step=1, ignore_data_errors=True)
assign_dict = {
    fitted_model.forecast_origin_column_name: "forecast_origin",
    fitted_model.forecast_column_name: predicted_column_name,
...
y_pred = np.array(X_rf[predicted_column_name])
...
rmse = np.sqrt(
    mean_squared_error(X_rf[target_column_name], X_rf[predicted_column_name])
)
print("[Test Data] \nRoot Mean squared error: %.2f" % rmse)
mae = mean_absolute_error(X_rf[target_column_name], X_rf[predicted_column_name])
print("mean_absolute_error score: %.2f" % mae)
print("MAPE: %.2f" % MAPE(X_rf[target_column_name], X_rf[predicted_column_name]))

run.log("rmse", rmse)
run.log("mae", mae)

file_name = "outputs/predictions.csv"
export_csv = X_rf.to_csv(file_name, header=True, index=False)  # added Index
